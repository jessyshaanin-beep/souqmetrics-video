...
const sources: Source[] = [
  {name: 'Shopify', file: 'shopify.png', position: [-2.35, 2.85, 0.3], rotation: [-0.18, 0.45, -0.15], delay: 0},
  {name: 'Meta', file: 'meta.png', position: [2.3, 2.45, -0.45], rotation: [0.25, -0.5, 0.18], delay: 4},
  {name: 'Google Ads', file: 'googleads.png', position: [-2.55, 0.25, -0.75], rotation: [0.3, 0.55, 0.1], delay: 8},
  {name: 'Snapchat', file: 'snapchat.svg', position: [2.55, 0.1, 0.55], rotation: [-0.28, -0.5, -0.2], delay: 12},
  {name: 'WooCommerce', file: 'woocommerce.svg', position: [-2.2, -2.55, -0.2], rotation: [0.25, 0.5, -0.12], delay: 16},
  {name: 'TikTok', file: 'tiktok.png', position: [2.25, -2.7, -0.65], rotation: [-0.28, -0.4, 0.15], delay: 20},
  {name: 'Amazon', file: 'amazon.png', position: [0.1, 3.35, -1.0], rotation: [0.2, -0.35, 0.08], delay: 24},
];
...
const PlatformCube: React.FC<{source: Source; index: number}> = ({source, index}) => {
  ...
  const start = 82 + source.delay;
  const pull = easeInOut(interpolate(frame, [start, start + 58], [0, 1], clamp));
  const float = 1 - pull;
  const position = source.position.map((value, axis) => {
    const drift = axis === 0 ? Math.sin((frame + index * 17) / 25) * 0.1 : axis === 1 ? Math.cos((frame + index * 11) / 28) * 0.11 : 0;
    return (value + drift) * float;
  }) as [number, number, number];
  position[2] += Math.sin(pull * Math.PI) * 2.15;
  const scale = interpolate(pull, [0, 0.75, 1], [1.08, 0.82, 0.03]);
  ...
  return (
    <group position={position} rotation={rotation} scale={scale} visible={pull < 0.995}>
      <RoundedBox args={[1.5, 1.5, 1.5]} radius={0.2} smoothness={5} castShadow receiveShadow>
        <meshPhysicalMaterial color="#f7f5f0" roughness={0.24} metalness={0.02} clearcoat={0.65} clearcoatRoughness={0.16} />
      </RoundedBox>
      <LogoFace file={source.file} size={0.82} z={0.757} />
    </group>
  );
};
  ...
  return (
    <group position={[0, interpolate(final, [0, 1], [0, 3.15]), 0]} rotation={[-0.12 + Math.sin(frame / 65) * 0.035, -0.27 + frame * 0.0018, 0.025 + Math.sin(frame / 80) * 0.02]} scale={scale}>
      <RoundedBox args={[3.0, 3.0, 3.0]} radius={0.38} smoothness={8} castShadow receiveShadow>
        <meshPhysicalMaterial color="#111416" roughness={0.2} metalness={0.34} clearcoat={1} clearcoatRoughness={0.12} envMapIntensity={0.7} />
      </RoundedBox>
      <mesh position={[0, 0, 1.508]}>
...
const Scene: React.FC = () => {
  ...
  return (
    <ThreeCanvas width={1080} height={1350} camera={{position: [cameraX, 0.08, 10.1], fov: 42, near: 0.1, far: 100}} shadows gl={{antialias: true, alpha: true}}>
      ...
      <directionalLight position={[6, -2, 4]} intensity={1.55} color="#bdeee5" />
